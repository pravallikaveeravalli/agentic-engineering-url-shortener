...
- [ ] T136 [US3] DS-B executed run with injected retry and compensation — `docs/evidence/ds-b/run.json`
  ...
  - **Progress (2026-09-22)**: **retry genuinely demonstrated, live, twice** — `DsBLiveRun`'s own real S3
    provider wrapper throws a real `IOException` on its first invocation only; `ProviderFailureTranslator`
    classifies it `UNAVAILABLE`, S3's own declared retryable set includes it, and `RetryRuling`'s real
    two-vote rule (`declared ∩ executor-proposed`) authorized a real retry both times attempted — attempt 2
    reached the real Claude CLI and succeeded (real `RETRY_ATTEMPT_*` telemetry captured,
    `docs/evidence/ds-b/attempts-1-3-finding.md`). **Compensation genuinely demonstrated, but as its own
    separate, real proof** (`DsBLiveRun#compensationOnlyDemonstration` — a real `Conductor.submit()` of the
    same aggregate-tier requirement, a real `RUN_CREATED` audit_record row, corrected by the real,
    already-unit-tested `CompensationHandler`, T090) rather than inside a run that also completed the tier,
    since no attempt reached S7 — precisely the plan's own pre-authorized End-Day-2-PM reduction
    (`docs/evidence/ds-b/t136-compensation-only.md`). Three capped live full-build attempts (1–3) all
...
- [ ] T145a [P] Load harness — `src/test/java/agentic/shortener/perf/LoadHarness.java`
  ...
  - **Progress (2026-09-22): DEFERRED**, owner's own standing decision (time-constrained) — NOT implemented, not attempted. Disclosed honestly as a real gap, not a silent skip: `docs/LIMITATIONS.md` §1. In a production rollout this harness and its own self-test would be built and run before any release decision relied on load figures.
- [ ] T145b Redirect latency and error-rate measurement — `src/test/java/agentic/shortener/perf/RedirectLoadIT.java`
  ...
  - **Progress (2026-09-22): DEFERRED**, owner's own standing decision (time-constrained) — NOT implemented, not attempted. PVT-001/PVT-004 remain binding thresholds with no task having measured them yet. Disclosed honestly: `docs/LIMITATIONS.md` §1.
- [ ] T145c Creation latency measurement — `src/test/java/agentic/shortener/perf/CreationLoadIT.java`
  ...
  - **Progress (2026-09-22): DEFERRED**, owner's own standing decision (time-constrained) — NOT implemented, not attempted. Disclosed honestly: `docs/LIMITATIONS.md` §1.
- [ ] T145d Analytics append-failure ceiling and first-bottleneck measurement — `src/test/java/agentic/shortener/perf/AnalyticsAppendCeilingIT.java`
  ...
  - **Progress (2026-09-22): DEFERRED**, owner's own standing decision (time-constrained) — NOT implemented, not attempted. NFR-SCA-003's own bottleneck-analysis measurement remains owed. Disclosed honestly: `docs/LIMITATIONS.md` §1.
...
- [x] T147 [P] Limitations and residual-risk disclosure — `docs/LIMITATIONS.md`
