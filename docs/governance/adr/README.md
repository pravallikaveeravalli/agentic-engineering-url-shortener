...
| [004-A3](./ADR-004-amendment-03-gemini-cli-for-live-demonstration-runs.md) | Gemini CLI (`agy`) as the live-demonstration transport — successor record amending ADR-004 | Accepted | High | `ClaudeCodeCliStageAiProvider` stays primary/production; T073a-f's live demo runs use `GeminiCliStageAiProvider` as a real, tested second transport. **Its own nesting-guard rationale is corrected by 004-A4** — see that record before citing this one's "Reason" section |
| [004-A4](./ADR-004-amendment-04-claude-cli-nesting-corrected.md) | Amendment 03's nesting-guard finding corrected; Claude CLI restored as the live/scenario transport — successor record amending ADR-004 and Amendment 03 | Accepted | High | The earlier "cannot spawn `claude` as a nested subprocess" finding was a Bash-tool allowlist artifact (a shell-redirect suffix breaking the match), not a structural guard — verified by a real, live, nested `claude` call via `ProcessBuilder`. Live/scenario runs (T132 onward) use `ClaudeCodeCliStageAiProvider`; `GeminiCliStageAiProvider` is retained as a demonstrated pluggable alternative |
